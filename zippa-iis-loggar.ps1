<#Windows PowerShell Code####################################################### 
 .SYNOPSIS  
    Archive Log Files: Manually specify folders or automatically parse all IIS  
    log file folders, and group by month or day. Archive them with 7-Zip, verify 
...
.DESCRIPTION  
    ...
    This script was originally written to specifically archive IIS log files, 
    but it has grown to handle files of any type, log files or otherwise. 
    ...
    7-Zip is required for this to work. By default this script looks for the  
    7-Zip executable, 7za.exe, in C:\7-Zip 
...
$SmtpClient.Host = "192.168.1.10" 
$MailMessage.from = ($ServerName + "@yourDomain.com") 
$MailMessage.To.add("ServerTeam@YourDomain.com") 
...
# day ->   Archive all past log files by day, excluding the current day  
...
if (!$UseWebAdministrationSnapIn) { 
    ...
    $Properties = @{ArchiveTargetName="Site 1";  
                    ArchiveTargetFolder="C:\inetpub\logs\LogFiles\W3SVC1\"} 
